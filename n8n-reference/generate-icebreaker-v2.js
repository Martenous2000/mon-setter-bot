...
if (hasPosts) {
  ...
  userContent = 'Prenom : ' + (prenom || '(aucun)') + '\n' + bloc2Context + '\nBio / Headline LinkedIn : ' + (headline || '(vide)')
    + (coverPictureUrl ? '\n(Une image de banniere est fournie en piece jointe pour le PS)' : '\n(Aucune image de banniere disponible pour le PS)');
}

let aiOutput = '';
try {
  const messageContent = [{ type: 'text', text: userContent }];
  // Si on a une image de banniere et qu'on est dans le cas "pas de post", on l'attache pour analyse visuelle reelle.
  if (!hasPosts && coverPictureUrl) {
    try {
